        ...
        while (read(fd, &cp, 1) > 0)
        {
            ...
            if (msgReceived)
            {
                ...
                case MAVLINK_MSG_ID_RC_CHANNELS:
                {
                    ...
                    if ((rc.chan1_raw > 1000) && (rc.chan7_raw != 1514) && (rc.chan8_raw < 2000))
                        rc_chans_raw_pub.publish(rc);

                    if (rc.chan5_raw < 1400)
                        state.offboard_state = 0;
                    else if ((rc.chan5_raw > 1400) && (rc.chan5_raw < 1600))
                        state.offboard_state = 0x01;
                    else if (rc.chan5_raw >= 1600)
...
void set_att_Callback(const mavlink_message::set_att_offboard &set_att)
{
    ...
    mavlink_set_att.q[2] = -set_att.q3; //from enu to ned
    mavlink_set_att.q[3] = -set_att.q4;
    ...
    mavlink_msg_set_attitude_target_encode_chan(0, 0, MAVLINK_COMM_0, &message, &mavlink_set_att);
